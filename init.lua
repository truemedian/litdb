...
local known_broken = {
	["shawwn/dax 0.0.1"] = true,
	["Bilal2453/discordia-interactions 0.0.2"] = true,
	["Bilal2453/vips 0.0.4"] = true,
	["Bilal2453/vips 1.1.18"] = true,
	["Bilal2453/vips 1.1.17"] = true,
	["Bilal2453/vips 1.1.16"] = true,
	["Bilal2453/vips 1.1.15"] = true,
	["Bilal2453/vips 1.1.14"] = true,
	["Bilal2453/vips 1.1.13"] = true,
	["Bilal2453/vips 1.1.12"] = true,
	["Bilal2453/vips 1.1.10"] = true,
	["Bilal2453/lit-vips 1.0.0"] = true,
	["Bilal2453/lua-vips 1.0.0"] = true,
	["truemedian/litdb-fixup 0.0.5"] = true,
	["truemedian/litdb-fixup 0.0.9"] = true,
	["truemedian/litdb-fixup 0.0.2"] = true,
	["truemedian/litdb-fixup 0.0.7"] = true,
	["truemedian/litdb-fixup 0.0.8"] = true,
	["truemedian/litdb-fixup 0.0.1"] = true,
	["truemedian/litdb-fixup 0.0.4"] = true,
	["truemedian/litdb-fixup 0.0.6"] = true,
	["truemedian/litdb-fixup 0.0.11"] = true,
}
...
for author in pairs(authors) do
	if not fs.accessSync(repo .. "/refs/tags/" .. author) then
...
local function reconnect()
	...
	repeat
		res, read, write = websocket.connect({
			host = "lit.luvit.io",
			port = 443,
			tls = true,
			pathname = "/",
...
local function fetch_hash(hash, objects)
	if objects[hash] then
		return
	end

	local data = fs.readFileSync(repo .. "/objects/" .. hash:sub(1, 2) .. "/" .. hash:sub(3))
	if data then
	...
	if root_hash then
			...
			if not objects[hash] then
				...
				if framed then
					...
					elseif kind == "commit" then
						...
						if not objects[tree_hash] then
							table.insert(hashes, tree_hash)
						end

						parents[tree_hash] = parents[hash] .. ".tree"

						for parent_hash in string.gmatch(body, "\nparent (%x+)") do
							if not objects[parent_hash] then
								table.insert(hashes, parent_hash)
							end
					...
					elseif kind == "tag" then
						...
						if not objects[object_hash] then
							table.insert(hashes, object_hash)
						end
		...
		if next(missing) then
			...
			for hash, object in pairs(objects) do
				...
				local success, err, num = fs.mkdirSync(repo .. "/objects/" .. hash:sub(1, 2))
				assert(success or num == "EEXIST", err)
				assert(fs.writeFileSync(repo .. "/objects/" .. hash:sub(1, 2) .. "/" .. hash:sub(3), deflated))
			end

			mkdirp(repo .. "/refs/tags/" .. package)
			assert(fs.writeFileSync(repo .. "/refs/tags/" .. package .. "/v" .. version, root_hash))
...
for author in fs.scandirSync(repo .. "/refs/tags") do
	mkdirp(repo .. "/refs/heads/" .. author)

	for package in fs.scandirSync(repo .. "/refs/tags/" .. author) do
		local package_path = repo .. "/refs/tags/" .. author .. "/" .. package
		local branch = author .. "/" .. package

		if not fs.accessSync(repo .. "/refs/heads/" .. branch) then
		...
		for version in fs.scandirSync(package_path) do
			...
			local tag_compressed =
				assert(fs.readFileSync(repo .. "/objects/" .. tag_hash:sub(1, 2) .. "/" .. tag_hash:sub(3)))
		...
		for i, version in ipairs(versions) do
			...
			if i > 1 then
				table.insert(data, "parent " .. commits[i - 1].hash)
			end
		...
		if #commits > 0 then
			assert(fs.writeFileSync(repo .. "/refs/heads/" .. author .. "/" .. package, commits[#commits].hash))
		end
	end
end
