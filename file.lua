...
-- You can pass the key for apply a `index` in table value, if it is a `table` (obviously).
-- If `value` are not a table, only uses it as value writer.
-- If there no key and value are a table and are no scope, 
-- then procced to apply to the next index in `#table + 1.`
--
-- The optional last argument or paramerameter is `notmakepath`, and the name don't sound nothing
-- to you, it's for not make a path is nothing rout exists.
-- @param module string
-- @param key? any
-- @param value? any
-- @param scope? string
...
-- The parameter module are explicit needed for create/import a file.
-- If you give scope module, then the function try to remove the module.
--
-- If you give any key as removal it tries to set the scope but not recommended.
-- @param module string
-- @param key? any
-- @param scope? string
-- @return code? boolean
