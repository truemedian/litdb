--[[lit-meta
	...
	description = "Provides dynamic ratelimiting an quick access to the ER:LC API."
]]
...
--[[
function erlua.Client(apikey, globalkey)
    local Client = {}
    Client.Connected = false
    if not apikey then
        return err("An API Key was not provided.")
    end
    local headers = {
        {"Server-Key", apikey}
    }
    ...
    local serverinfo, res = getBody("server", headers)
    if serverinfo then
        Client.Connected = true
        Client.Server = serverinfo
        print("[ERLua] Connected to " .. serverinfo.Name)
        function Client.Players()
            getBody("server/players", headers)
        end
        return Client
    else
        Client.Connected = false
