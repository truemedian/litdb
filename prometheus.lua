...
local LOG_ERROR_HEAD = " ERROR "

local Dict = require('./dict').Dict
...
--   logger: (function Logger.log() end) debugging logger to collect module errors
--
-- Returns:
--   an object that should be used to register metrics.
function Prometheus.init(prefix, logger)
	...
	self:counter("prometheus_metric_errors_total",
	"Number of luvit-prometheus errors")
	self.dict:set("prometheus_metric_errors_total", 0)
	return self
end

function Prometheus:log_error(...)
	self.logger.log(LOG_ERROR_HEAD, ...)
	self.dict:inc("prometheus_metric_errors_total", 1)
end
...
--
-- This function should be used to expose the metrics on an HTTP page.
