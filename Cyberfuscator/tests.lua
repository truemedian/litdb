...
cyber.colors.enabled = not noColors;

-- Apply Obfuscation Pipeline
local pipeline = cyber.Pipeline:new({
...
local presets = cyber.Presets;
local testdir = "./tests/"
local failed = {};
cyber.Logger.logLevel = cyber.Logger.LogLevel.Error;
local fc = 0;
for i, filename in ipairs(scandir(testdir)) do
	...
	print(cyber.colors("[CURRENT] ", "magenta") .. filename);
	for name, preset in pairs(presets) do
		...
		pipeline = cyber.Pipeline:fromConfig(preset);
		...
		if funcb == nil then
			print(cyber.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name .. ", Invalid Lua!");
			...
			if not validated then
				print(cyber.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name);
				print("[OUTA]    ",    outa);
				print("[OUTB]    ", outb);
				print("[SOURCE]", obfuscated);
				fc = fc + 1;
...
if fc < 1 then
	print(cyber.colors("[PASSED]  ", "green") .. "All tests passed!");
	return 0;
else
	print(cyber.colors("[FAILED]  ", "red") .. "Some tests failed!");
	if ciMode then
		error("Test Failed!")
	end
	return -1;
